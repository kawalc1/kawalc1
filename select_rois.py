...
if __name__ == '__main__':
    ...
    while True:
        ...
        shape = zoom_roi.shape
        zoom_shape = (int(shape[1] * zoom_factor), int(shape[0] * zoom_factor))
        zoomed_image = numpy.array(Image.fromarray(zoom_roi).resize(zoom_shape))
        crosshair_size = 30
        ...
        if key == ord("s") and len(refPt) == 2:
            print(str(
                "[" + str(refPt[0][1] * 2) + "," + str(refPt[1][1] * 2) + ", " + str(refPt[0][0] * 2) + "," + str(
                    refPt[1][0] * 2) + "]"))
