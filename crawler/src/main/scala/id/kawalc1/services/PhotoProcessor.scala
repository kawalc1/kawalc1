...
class PhotoProcessor(kawalPemiluClient: KawalPemiluClient)(implicit
  ...
  private def transform[A, B](sourceDb: SQLiteProfile.backend.Database,
  ...
  private def batchTransform[A, B, C <: Table[A]](sourceDb: SQLiteProfile.backend.Database,
                                                  targetDb: SQLiteProfile.backend.Database,
  ...
  def fetch(sourceDb: SQLiteProfile.backend.Database,
            targetDb: SQLiteProfile.backend.Database,
            client: KawalC1Client,
            params: BatchParams): Long = {

    batchTransform[KelurahanId, Seq[SingleTps], KelurahanTable](sourceDb,
                                                                targetDb,
                                                                client,
  ...
  def processDetections(sourceDb: SQLiteProfile.backend.Database,
                        targetDb: SQLiteProfile.backend.Database,
                        client: KawalC1Client,
                        params: BatchParams)(implicit pw: PrintWriter): Long = {
    pw.println("kelurahan,tps,photo,response_code,config,pas1,pas2,jumlah,tidak_sah,php_jumlah,confidence,hash,similarity,aligned,roi")
    batchTransform[AlignResult, CombiResult, AlignResults](
      sourceDb,
      targetDb,
      client,
      ResultsTables.alignResultsQuery.filter(x => x.config === "digit_config_ppwp_scan_halaman_1_2019.json"),
      streamDetections,
      writeToCsv,
      params
    )
  ...
  def toCsv(r: CombiResult): String = {
    val resp                                  = r.response
    val identifier                            = s"${r.kelurahan},${r.tps},${r.photo},${r.responseCode},${resp.configFile}"
    ...
    val value  = maybeMap.flatMap(_.get("confidence")).getOrElse(0.0)
    val common = s"$value,${resp.hash},${resp.similarity},${resp.transformedUrl},${resp.digitArea}"
  ...
  def streamDetections(tps: Seq[AlignResult], threads: Int, client: KawalC1Client): Future[Seq[CombiResult]] = {
    streamResults(tps, processSingleDetection, threads, client)
  }

  val rand = new scala.util.Random

  def processSingleDetection(align: AlignResult, client: KawalC1Client): Future[CombiResult] = {
    client
      .detectNumbers(kelurahan = align.id,
                     tps = align.tps,
                     photoName = align.photo.replace("http://lh3.googleusercontent.com/", "") + "=s1280")
      .map {
        case Right(e) =>
          CombiResult(align.id, align.tps, align.photo, 200, Serialization.write(e), e)
        case Left(error) =>
          logger.warn(s"Error digitizing: $error")
          val emptyResponse = CombiResponse(Some("error"), Some(0.0), Some("hash"), Some("digit"), None, Some(align.config))
          CombiResult(align.id, align.tps, align.photo, error.code, error.response, emptyResponse)
      }
  }
